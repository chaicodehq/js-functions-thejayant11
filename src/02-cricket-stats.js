...
export const calcStrikeRate = (runs, balls) => {
  // Your code here
  if (typeof runs !== "number" || typeof balls !== "number" || balls <= 0 || runs < 0) return 0
  return Number(((runs / balls) * 100).toFixed(2))
};

export const calcEconomy = (runsConceded, overs) => {
  // Your code here
  if (typeof runsConceded !== "number" || typeof overs !== "number" || overs <= 0 || runsConceded < 0) return 0
...
export const calcBattingAvg = (totalRuns, innings, notOuts = 0) => {
  ...
  return Number((totalRuns / outs).toFixed(2))
};
